...
import { ETAPAS, ETAPA_COLORES, type Proyecto, type Documento, type PartidaFinanciera, type Prospecto } from '@/types'
...
export default function ProyectoDetalle({
  ...
  const [nuevoProspecto, setNuevoProspecto] = useState({ nombre: '', telefono: '', tipo_credito: 'Infonavit', temperatura: 'frio' })
  ...
  async function avanzarEtapa() {
    ...
    const faltantesOblig = docs.filter(d => d.etapa === proyecto.etapa_actual && d.estado === 'faltante' && d.requerido === 'obligatorio')
    if (faltantesOblig.length > 0) {
      alert(`No puedes avanzar: hay ${faltantesOblig.length} documento(s) obligatorio(s) faltantes en esta etapa.`)
      return
    }
    const nuevaEtapa = proyecto.etapa_actual + 1
    await supabase.from('proyectos').update({ etapa_actual: nuevaEtapa }).eq('id', proyecto.id)
    router.refresh()
  }
  ...
  async function cambiarEstadoDoc(docId: string, nuevoEstado: 'faltante' | 'tramite' | 'listo') {
    setSavingDoc(docId)
  ...
  const pctDocs = Math.round((docsListos / docs.length) * 100)
  const faltantesObligActual = docs.filter(d => d.etapa === proyecto.etapa_actual && d.estado === 'faltante' && d.requerido === 'obligatorio').length
  ...
  async function agregarProspecto() {
    ...
    const { data } = await supabase.from('prospectos').insert({
  ...
  async function cambiarTemperatura(id: string, temperatura: string) {
    await supabase.from('prospectos').update({ temperatura }).eq('id', id)
    setProspectos(prev => prev.map(p => p.id === id ? { ...p, temperatura: temperatura as any } : p))
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {proyecto.zona} · {proyecto.metros2}m² · {proyecto.recamaras} rec / {proyecto.banos} baño
              {proyecto.fecha_inicio && ` · Inicio: ${new Date(proyecto.fecha_inicio).toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' })}`}
            </p>
          </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-[11px] text-gray-400 mb-1.5">
            <span>Progreso general</span><span>{progreso}%</span>
          </div>
          <div className="flex gap-1">
            {ETAPAS.map(e => (
              <div key={e.num} className="flex-1 h-1.5 rounded-full transition-all"
                   style={{
                     background: e.num < proyecto.etapa_actual
                       ? '#C0271A'
      ...
      {tab === 'etapas' && (
        <div className="flex flex-col gap-2">
          {ETAPAS.map(e => {
            ...
            return (
              <div key={e.num} className={`bg-white rounded-xl border p-4 flex items-center gap-4 ${estado === 'actual' ? 'border-amber-200' : 'border-gray-200'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${
                  estado === 'done' ? 'bg-green-100 text-green-700' :
                  estado === 'actual' ? 'bg-amber-100 text-amber-700' :
                ...
                {estado === 'done' && (
                  <span className="text-[10px] bg-green-50 text-green-600 px-2 py-1 rounded-full">Completo</span>
                )}
              </div>
            )
          })}
      ...
      {tab === 'documentos' && (
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-3 flex items-center gap-4">
            <div className="flex-1">
              <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                ...
                <span className="font-medium" style={{ color: '#C0271A' }}>{pctDocs}%</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full">
                <div className="h-2 rounded-full transition-all" style={{ width: `${pctDocs}%`, background: '#C0271A' }} />
              </div>
            </div>
            <div className="text-xs text-gray-400">{docsListos} de {docs.length} listos</div>
          </div>
          ...
          {faltantesObligActual > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-3 flex items-center gap-2">
              ...
              <p className="text-xs text-amber-700">
                Avance bloqueado: {faltantesObligActual} documento{faltantesObligActual > 1 ? 's' : ''} obligatorio{faltantesObligActual > 1 ? 's' : ''} faltante{faltantesObligActual > 1 ? 's' : ''} en la etapa actual.
              </p>
            </div>
          )}
          ...
          {ETAPAS.map(e => {
            ...
            const etapaEstado = e.num < proyecto.etapa_actual ? 'done' : e.num === proyecto.etapa_actual ? 'actual' : 'pendiente'
                ...
                <button className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50 transition"
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-semibold ${
                      etapaEstado === 'done' ? 'bg-green-100 text-green-700' :
                      etapaEstado === 'actual' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-400'
                    }`}>
                  ...
                  <div className="flex items-center gap-2">
                    {todos ? (
                      <span className="text-[10px] bg-green-50 text-green-600 px-2 py-0.5 rounded-full">Completo</span>
                    ) : faltOblig > 0 ? (
                      <span className="text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded-full">{faltOblig} obligatorio{faltOblig > 1 ? 's' : ''} faltante{faltOblig > 1 ? 's' : ''}</span>
                    ) : (
                      <span className="text-[10px] bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">Opcional pendiente</span>
                    )}
                  ...
                  <div className="border-t border-gray-100 divide-y divide-gray-100">
                    {docsEtapa.map(d => (
                      <div key={d.id} className="flex items-center gap-3 px-4 py-3">
                        <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 cursor-pointer border transition ${
                          d.estado === 'listo' ? 'bg-green-500 border-green-500' : 'border-gray-300'
                        }`}
                          onClick={() => cambiarEstadoDoc(d.id, d.estado === 'listo' ? 'faltante' : 'listo')}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className={`text-[9px] px-1.5 py-0.5 rounded ${
                              d.requerido === 'obligatorio' ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'
                            }`}>
                              {d.requerido}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <select value={d.estado}
                            ...
                            onChange={ev => cambiarEstadoDoc(d.id, ev.target.value as any)}
      ...
      {tab === 'finanzas' && (
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="flex flex-col gap-2">
              {parts.filter(p => p.tipo === 'egreso').map(p => (
                <div key={p.id} className="flex items-center gap-2">
                  <input type="checkbox" checked={p.aplica}
                    onChange={e => toggleAplica(p.id, e.target.checked)}
                    className="accent-red-600 w-3 h-3 flex-shrink-0" />
                  <span className={`text-xs flex-1 ${!p.aplica ? 'text-gray-300 line-through' : 'text-gray-600'}`}>
                    {p.concepto}
                  </span>
                  <input
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="flex flex-col gap-2">
                {parts.filter(p => p.tipo === 'ingreso').map(p => (
                  <div key={p.id} className="flex items-center gap-2">
                    <input type="checkbox" checked={p.aplica}
                      onChange={e => toggleAplica(p.id, e.target.checked)}
                      className="accent-red-600 w-3 h-3 flex-shrink-0" />
            ...
            <div className="bg-white rounded-xl border-2 p-4 flex items-center justify-between"
                 style={{ borderColor: utilidad >= 0 ? '#16a34a' : '#dc2626' }}>
              <div>
                <p className="text-xs text-gray-500">Utilidad estimada</p>
                <p className={`text-2xl font-bold mt-0.5 ${utilidad >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {fmt(utilidad)}
                </p>
              </div>
              <div className="text-right">
      ...
      {tab === 'prospectos' && (
        <div>
          <div className="flex justify-between items-center mb-3">
            <p className="text-sm text-gray-500">{prospectos.length} prospecto{prospectos.length !== 1 ? 's' : ''} registrado{prospectos.length !== 1 ? 's' : ''}</p>
            <button onClick={() => setShowAddProspecto(true)}
          ...
          {showAddProspecto && (
            <div className="bg-white rounded-xl border border-gray-200 p-4 mb-3">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <input value={nuevoProspecto.nombre} onChange={e => setNuevoProspecto(p => ({ ...p, nombre: e.target.value }))}
                  placeholder="Nombre completo *"
                  className="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none" />
                <input value={nuevoProspecto.telefono} onChange={e => setNuevoProspecto(p => ({ ...p, telefono: e.target.value }))}
                  placeholder="Teléfono"
                  className="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none" />
                <select value={nuevoProspecto.tipo_credito} onChange={e => setNuevoProspecto(p => ({ ...p, tipo_credito: e.target.value }))}
                ...
                <select value={nuevoProspecto.temperatura} onChange={e => setNuevoProspecto(p => ({ ...p, temperatura: e.target.value }))}
                  className="px-3 py-2 rounded-lg border border-gray-200 text-sm bg-white focus:outline-none">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowAddProspecto(false)}
                  className="flex-1 py-2 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50">
                  Cancelar
                </button>
                <button onClick={agregarProspecto} disabled={!nuevoProspecto.nombre}
              ...
              {prospectos.map(p => (
                <div key={p.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      {p.tipo_credito} {p.estado_credito ? `· ${p.estado_credito}` : ''} {p.telefono ? `· ${p.telefono}` : ''}
                    </div>
                  </div>
                  <select value={p.temperatura}
                    onChange={e => cambiarTemperatura(p.id, e.target.value)}
                    className={`text-[11px] px-2 py-1 rounded-full border-0 font-medium cursor-pointer focus:outline-none ${TEMP_BADGE[p.temperatura]}`}>
                    <option value="caliente">🔥 Caliente</option>
                    <option value="tibio">〰 Tibio</option>
                    <option value="frio">❄ Frío</option>
                  </select>
